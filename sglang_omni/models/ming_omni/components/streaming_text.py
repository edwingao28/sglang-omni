...
TokenSplitFn = Callable[[str, int], tuple[str, str]]
...
class SegmenterState:
    def __init__(
        ...
        token_split_fn: TokenSplitFn | None = None,
    ) -> None:
        self.config = config
        self.token_count_fn = token_count_fn
        self.token_split_fn = token_split_fn or split_whitespace_tokens
        self._buffer = ""
        self._segment_id = 0
    ...
    def _emit_max_window(self, *, now_ms: int) -> TextSegment:
        text, remainder = self.token_split_fn(
            self._buffer, self.config.segment_max_tokens
        )
        if not text:
            text, remainder = self._buffer, ""
