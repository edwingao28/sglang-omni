...
class Code2WavScheduler(StreamingVocoderBase[Code2WavStreamState, "list[int]"]):
    ...
    def select_step_participants(self) -> list[tuple[str, Code2WavStreamState]]:
        ...
        due: dict[
            tuple[int, int], list[tuple[str, Code2WavStreamState]]
        ] = {}
        ...
        if first_ready:
            # Note (wenyao): first windows never wait, but still batch per
            # bucket so one trim scalar holds; leftovers fire on the re-pump.
    ...
    def run_step(
        ...
        for sub in plan:
            ...
            wav, execution_metadata = self._forward_codes(codes, graph_eligible=True)
            ...
            if trim:
                wav = wav[..., trim:]
            # Note (wenyao): consume the borrowed graph buffer before returning.
