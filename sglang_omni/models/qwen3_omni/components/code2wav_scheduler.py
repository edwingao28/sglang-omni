    ...
    def _forward_codes(
        ...
        *,
        ...
        if (
            ...
            # EOS is materialized synchronously before the next replay. Only
            # exact final keys absent from the baseline use the optional pool.
...
def _build_eos_cuda_graph_runner(
    ...
    *,
    ...
    """Build optional exact tail keys without borrowing the baseline pool.

    The baseline's reserved graph footprint is charged before this second
    runner is built. Its batch-1 tier is atomic: failure releases only this
    optional pool and preserves the baseline matrix and batching policy.
    """
    ...
    # Keep the original baseline startup marker unambiguous for admission.
    logger.info(
