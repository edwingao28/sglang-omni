...
that were moved out of Phase 1. The PR restores two semantic-conditioning
paths and keeps source selection explicit until quality and latency are
benchmarked side by side.

| File | Role | PR decision | Why |
| --- | --- | --- | --- |
| `semantic_encoder.py` | Standalone semantic conditioning | Restore now as an explicit source and validation baseline | It may be the canonical image-generation conditioning path and must be compared against thinker-fused quality/latency. |
...
- Image requests must set `image_generation.semantic_source` explicitly.
