...
- **`modalities: ["text", "audio"]` is rejected on a text-only server.** The request fails up front with HTTP 400 `Audio output is unavailable for this pipeline; request "modalities": ["text"].` instead of quietly returning text without audio. Use a speech-mode server (without `--text-only`) to get audio output.
