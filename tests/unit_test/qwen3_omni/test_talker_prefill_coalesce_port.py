...
def _execute(nodes, namespace):
    ...
    exec(
        compile(ast.fix_missing_locations(tree), "<actual-production-body>", "exec"),
        namespace,
    )
...
def _scheduler_types():
    ...
    gate = _methods(
        ROOT / "sglang_omni/scheduling/omni_scheduler.py",
        "OmniScheduler",
        {"get_new_batch_prefill"},
    )
    base = ast.ClassDef(
        name="OmniScheduler", bases=[], keywords=[], body=gate, decorator_list=[]
    )
    talker = ast.ClassDef(
        name="QwenTalkerScheduler",
        bases=[ast.Name(id="OmniScheduler", ctx=ast.Load())],
        keywords=[],
        body=_methods(
            MODEL / "talker_scheduler.py",
            "QwenTalkerScheduler",
            {"get_new_batch_prefill", "_admin_model_info"},
        ),
        decorator_list=[],
    )
    ns = {"_Upstream": Upstream, "time": clock, "NextBatchPlan": lambda **kw: NS(**kw)}
    _execute([base, talker], ns)
    ns["OmniScheduler"]._admin_model_info = lambda self: {
        "success": True,
        "data": {"existing": 7},
    }
...
class GateTests(unittest.TestCase):
    ...
    def test_disabled_has_no_hold(self):
        ...
        self.assertEqual(
            len(s.get_new_batch_prefill(self.running).batch_to_run.reqs), 1
        )
    ...
    def test_timeout_does_not_restart_and_fifo_is_preserved(self):
        ...
        self.assertEqual(
            s.get_new_batch_prefill(self.running).batch_to_run.reqs, [a, b]
        )
    ...
    def test_partial_admission_preserves_leftover_deadline(self):
        ...
        s.admit_limit = 1
        self.assertEqual(
            s.get_new_batch_prefill(self.running).batch_to_run.reqs, rows[:1]
        )
        self.assertEqual(
            s.get_new_batch_prefill(self.running).batch_to_run.reqs, rows[1:2]
        )
    ...
    def test_admin_policy_is_detached_and_serializable(self):
        ...
        self.assertEqual(
            msgpack.unpackb(msgpack.packb(info), strict_map_key=True), info
        )
...
class FactoryTests(unittest.TestCase):
    ...
    def test_scratch_option_binds_before_capture_on_identical_initialization_path(self):
        for skip_scratch in [False, True]:
            for graph_enabled in [False, True]:
                with self.subTest(
                    skip_scratch=skip_scratch, graph_enabled=graph_enabled
                ):
                    self._run_factory(
                        4,
                        lookahead=True,
    ...
    def _run_factory(
        self, target, lookahead=None, skip_scratch=None, graph_enabled=False
    ):
        observed = {}
        cfg = NS(
        ...
        exports = dict(
            get_tokenizer=lambda *a, **kw: object(),
            current_platform=NS(enable_talker_graph=lambda: True),
            make_talker_scheduler_adapters=adapters,
            QwenTalkerModelRunner=lambda *a, **kw: NS(**kw),
            QwenTalkerScheduler=FakeScheduler,
            configure_talker_server_args=lambda *a, **kw: graph_enabled,
        ...
        ns.update(
            build_generation_batch_overrides=lambda **kw: {},
            ...
            avail_gpu_mem=lambda *a: 1,
            get_process_gpu_memory_bytes=lambda *a: 0,
            format_bytes_gib=str,
            os=NS(getpid=lambda: 1),
            logger=NS(info=lambda *a: None),
        )
        _execute(
            [_function(MODEL / "stages.py", "create_talker_ar_executor_from_config")],
            ns,
        )
        ...
        for factory in [stage, ns["create_talker_scheduler"]]:
            ...
            self.assertIs(
                inspect.signature(factory).parameters["enable_async_decode"].default,
                False,
            )
            self.assertEqual(
                inspect.signature(factory)
        ...
        self.assertFalse(
            any(k.startswith("prefill_coalesce") for k in observed["adapters"])
        )
