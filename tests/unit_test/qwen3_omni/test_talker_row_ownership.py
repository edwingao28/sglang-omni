...
from tests.unit_test.fakes import init_terminal_output_state

POOL_SIZE = 8
# Note (wenyao): deliberately not equal to the batch row index, so a positional
# read cannot pass.
...
def test_retract_snapshot_runs_on_the_flag_only_path(monkeypatch: Any) -> None:
    # Note (wenyao): _retract_running_requests requeues without the is_retracted
    # kwarg, so the snapshot must also trigger off Req.reset_for_retract's flag alone.
    n, hidden, code_groups = 1, 3, 2
    model = _fake_model(n, hidden, code_groups)
    runner = _runner(model)
...
def test_upstream_add_request_to_queue_still_takes_is_retracted() -> None:
    # Note (wenyao): an upstream version bump dropping is_retracted must fail here
