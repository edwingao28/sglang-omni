...
def test_immediate_abort_survivors_keep_launch_token_rows(removed, finished):
    ...
    launch = SimpleNamespace(requests=[SimpleNamespace(request_id=req.rid) for req in reqs])
...
def test_default_off_sync_publishes_codec_before_feedback():
    ...
    observed = []
    runner._outbox.put = lambda message: observed.append(len(data.pending_feedback_queue))
    runner._emit_code_chunks_and_feedback(schedule_batch=_sched_batch(1), requests=[_req_wrap(data)])
    assert observed == [0]
    assert len(data.pending_feedback_queue) == 1
...
def test_async_resolve_preserves_staged_host_token_ids():
    ...
    output = ModelRunnerOutput(outputs={}, next_token_ids=device_ids, host_token_ids=staged_ids)
    scheduler = object.__new__(OmniScheduler)
    scheduler._model_runner = SimpleNamespace(execute_resolve=lambda pending: output)
    emitted = []
    scheduler._emit_stream_output = lambda *args, **kwargs: emitted.append((args, kwargs))
