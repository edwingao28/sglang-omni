...
def test_qwen_prefill_debug_snapshot_serializes_logical_hidden_rows_and_logits():
    static_embed = torch.arange(18, dtype=torch.bfloat16).reshape(6, 3)
    static_layer = torch.arange(100, 118, dtype=torch.bfloat16).reshape(6, 3)
    capture = StaticAuxHiddenCapture(
        buffers=[static_embed, static_layer],
        hook_handles=[],
        max_tokens=6,
    ...
    output_processor = SGLangOutputProcessor(
        ...
        capture_hidden_layers=[0, 24],
        model=SimpleNamespace(_omni_aux_hidden_capture=capture),
        capture_prefill_debug_snapshot=True,
    )
    scheduler_output = SchedulerOutput(
        requests=[SchedulerRequest(request_id="snapshot-1")],
        batch_data=SimpleNamespace(
            forward_mode=SimpleNamespace(is_extend=lambda: True),
            reqs=[SimpleNamespace(extend_range=SimpleNamespace(length=2))],
        ),
    )
    logits = torch.tensor([[1.25, -2.5, 3.75]], dtype=torch.bfloat16)
    model_output = SimpleNamespace(
    ...
    output_processor.process(model_output, scheduler_output)
    snapshot = output_processor.prefill_debug_snapshot()

    assert snapshot is not None
    assert snapshot["logical_rows"] == 2
    assert snapshot["requests"][0]["request_id"] == "snapshot-1"
    assert snapshot["requests"][0]["logical_rows"] == 2
    assert snapshot["requests"][0]["next_token_id"] == 7

    def decode_tensor(payload: dict) -> torch.Tensor:
        raw = base64.b64decode(payload["data"])
        values = torch.frombuffer(bytearray(raw), dtype=torch.uint8)
        return values.view(getattr(torch, payload["dtype"])).reshape(payload["shape"])

    encoded_hidden = snapshot["requests"][0]["hidden_states"]
    torch.testing.assert_close(decode_tensor(encoded_hidden["embed"]), static_embed[:2])
    torch.testing.assert_close(decode_tensor(encoded_hidden["24"]), static_layer[:2])
    torch.testing.assert_close(
        decode_tensor(snapshot["requests"][0]["next_token_logits"]), logits[0]
    )

    # A later prefill replaces the snapshot instead of retaining stale tensors.
    static_embed.add_(100)
    static_layer.add_(200)
    scheduler_output.requests[0].request_id = "snapshot-2"
    output_processor.process(model_output, scheduler_output)
    refreshed = output_processor.prefill_debug_snapshot()
    assert refreshed["logical_rows"] == 2
    assert refreshed["requests"][0]["request_id"] == "snapshot-2"
    torch.testing.assert_close(
        decode_tensor(refreshed["requests"][0]["hidden_states"]["embed"]),
        static_embed[:2],
    )


def test_qwen_prefill_debug_snapshot_slices_mixed_batch_by_request():
    static_embed = torch.arange(20, dtype=torch.bfloat16).reshape(5, 4)
    static_layer = torch.arange(100, 120, dtype=torch.bfloat16).reshape(5, 4)
    capture = StaticAuxHiddenCapture(
        buffers=[static_embed, static_layer],
        hook_handles=[],
        max_tokens=5,
    ...
    scheduler_output = SchedulerOutput(
        requests=[
            SchedulerRequest(request_id="audio"),
            SchedulerRequest(request_id="visual"),
        ],
        batch_data=SimpleNamespace(
            forward_mode=SimpleNamespace(is_extend=lambda: True),
            reqs=[
                SimpleNamespace(extend_range=SimpleNamespace(length=3)),
                SimpleNamespace(extend_range=SimpleNamespace(length=2)),
            ],
        ),
    )
    logits = torch.arange(6, dtype=torch.bfloat16).reshape(2, 3)
    model_output = SimpleNamespace(
    ...
    output_processor.process(model_output, scheduler_output)
    debug_snapshot = output_processor.prefill_debug_snapshot()
    ...
    assert debug_snapshot["logical_rows"] == 5
    by_id = {item["request_id"]: item for item in debug_snapshot["requests"]}
    assert by_id["audio"]["logical_rows"] == 3
    assert by_id["visual"]["logical_rows"] == 2
    torch.testing.assert_close(
        decode_tensor(by_id["audio"]["hidden_states"]["embed"]),
        static_embed[:3],
    )
    torch.testing.assert_close(
        decode_tensor(by_id["visual"]["hidden_states"]["embed"]),
        static_embed[3:5],
    )
    torch.testing.assert_close(
        decode_tensor(by_id["audio"]["hidden_states"]["24"]),
        static_layer[:3],
    )
    torch.testing.assert_close(
        decode_tensor(by_id["visual"]["next_token_logits"]), logits[1]
    )
