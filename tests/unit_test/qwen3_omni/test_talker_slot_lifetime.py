...
def _drive_to_second_retract(
    ...
    # Note (wenyao): two emits with no consume between them, because a retract
    # requeue re-prefills and emits again while the previous frame is still owed, so
    # one snapshot cannot cover the count and the request reaches a second retract
    # with feedback still pending.
...
def test_second_retract_cannot_read_a_recycled_slot(monkeypatch: Any) -> None:
    ...
    # Note (wenyao): asserted at the invariant site, because the scheduler hook
    # contains this raise into a single-request abort, covered by the containment
    # test below.
    req.is_retracted = True
    req.req_pool_idx = None
    ...
    # Note (wenyao): the stranger's row must not reach the replay consumer by any
    # route, not just the one the raise above blocks.
    assert data.retracted_feedback_embed is None
    assert (
    ...
    # Note (wenyao): rA's own frame was 6.0; the stranger's row would surface as 99.0
    # raw or 109.0 combined with the text row.
...
def test_retract_clears_the_slot_index_when_a_snapshot_is_already_held(
    ...
    # Note (wenyao): the pool slot is freed on this retract too, so the early return
    # for an unconsumed snapshot must still retire the index.
    model = _model()
    runner = _runner(model)
    req = _req("rA", POOL_IDX)
    data = _data(req)
...
def test_emit_after_retract_rearms_the_slot_index(monkeypatch: Any) -> None:
    # Note (wenyao): the requeued request prefills into a fresh pool index, so
    # invalidation must not outlive the next emit.
    model = _model()
    runner = _runner(model)
    scheduler = _retract_scheduler(runner, monkeypatch)
    req = _req("rA", POOL_IDX)
...
def test_pause_path_retract_retires_the_slot_index(monkeypatch: Any) -> None:
    # Note (wenyao): upstream ``retract_all`` (the pause path) requeues without the
    # is_retracted kwarg, so the hook's disjunction with Req.is_retracted has to carry
    # the snapshot and the invalidation on its own.
    model = _model()
    runner = _runner(model)
    req = _req("rA", POOL_IDX)
    data = _data(req)
...
def test_failed_retract_snapshot_fails_one_request_not_the_stage(
    ...
    # Note (wenyao): the retract hook runs inside get_next_batch_to_run, which the
    # event loop calls outside any try, so an escaping error would take the scheduler
    # thread and every co-resident request with it.
    model = _model()
    runner = _runner(model)
...
def test_replay_past_the_single_snapshot_raises() -> None:
    # Note (wenyao): the slot table keeps only the newest emit per request, so a
    # retract can recover exactly one row however many are owed.
    model = _model()
    runner = _runner(model)
    req = _req("rA", POOL_IDX)
    data = _data(req)
