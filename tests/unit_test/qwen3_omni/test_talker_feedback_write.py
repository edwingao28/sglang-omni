...
def test_retract_override_ignores_the_batch_pool_indices() -> None:
    # Note (wenyao): a snapshotted row has no live pool slot, so the batch's indices
    # no longer name where each row's feedback lives and a wrong-but-in-bounds index
    # would read a stranger's row.
    n, hidden = 2, 3
    model = _fake_model(n, hidden)
    runner = _runner(model)
...
def test_sparse_write_ignores_the_batch_pool_indices() -> None:
    # Note (wenyao): a starved row drops out of the gather, so batch row i and
    # gather row i stop lining up and the batch's indices cannot be used
    # positionally.
    n, hidden = 3, 3
    model = _fake_model(n, hidden)
    runner = _runner(model)
...
def test_write_reads_feedback_from_the_requests_own_slot() -> None:
    ...
    # Note (wenyao): batch order and pool order deliberately disagree, so a
    # positional read would pick up the wrong request's row.
    model._feedback_slots[POOL_IDS[0]] = torch.full((hidden,), 1.0)
    model._feedback_slots[POOL_IDS[1]] = torch.full((hidden,), 2.0)
    requests = [
