...
"""CPU fake-backend checks; these do not establish real decoder parity."""
...
BASE_KEYS = mod._batched_graph_keys(10, 25, 16, 2)
EXTRA_KEYS = tuple(
...
def _baseline():
    ...
    assert runner.stats()["build"]["published_graph_count"] == 22
...
def _optional(monkeypatch, model, baseline, backend, *, max_frames=35):
    ...
    class _Builder:
        ...
        def build(cls, model, **kwargs):
            ...
            return Code2WavCudaGraphRunner.build(model, cuda_api=backend, **kwargs)
...
def test_optional_pool_captures_only_missing_keys_and_charges_reserved_baseline(
    ...
    assert len(EXTRA_KEYS) == 30
    # Stage500 - original model100 - baseline reserved delta280 =120,
    # whereas subtracting only current allocated140 would admit360.
    assert calls[0]["graph_memory_budget_bytes"] == 120
    assert optional.stats()["memory"]["graph_budget_bytes"] == 120
    assert optional.stats()["build"]["published_graph_count"] == 30
    ...
    assert backend.capture_calls == 30
    assert backend.warmup_iterations == [3] * 30
    # Multiple nonzero inputs per shape exercise copy/replay, beyond the
    # existing startup zero-input equivalence check, through the CPU fake.
...
def test_optional_startup_failure_preserves_baseline_graphs_and_batching(
    ...
    assert baseline.run(_codes(baseline_backend, 1, 32)).execution_mode == "cuda_graph"
...
def test_eos_before_payload_emits_tail_once_then_terminal_result():
    scheduler, _, _, optional = _scheduler()
    state = _state(34)
    scheduler._stream_states["a"] = state
    first = scheduler.on_stream_done_before_payload("a")
    assert [message.type for message in first] == ["stream"]
    assert scheduler.on_stream_done_before_payload("a") == []
    scheduler._stream_payloads["a"] = make_qwen_payload(request_id="a")
    last = scheduler.on_stream_done("a")
    assert [message.type for message in last] == ["result"]
...
def test_factory_leaves_baseline_matrix_unchanged_and_plumbs_eos_flag(
    ...
    baseline_messages = [
        m for m in caplog.messages if m.startswith("Code2Wav CUDA graph startup stats=")
    ]
    optional_messages = [
        m
    ...
    if enabled:
        ...
        assert report["baseline_published_graph_count"] == 22
        assert report["optional_published_keys"] == report["optional_requested_keys"]
        assert len(report["optional_published_keys"]) == 30
