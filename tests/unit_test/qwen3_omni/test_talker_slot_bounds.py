...
def test_emit_scatter_covers_every_allocatable_index() -> None:
    ...
    # Note (wenyao): row 0 is the pool's reserved pad row, never a real request's.
...
def test_retract_snapshot_reads_top_pool_index() -> None:
    ...
    # Note (wenyao): the snapshot must be a fresh allocation, not a view of the
    # reusable slot.
...
def test_consume_gather_row_zero_fallback_is_discarded() -> None:
    # Note (wenyao): a snapshotted request has no pool index, so the gather falls
    # back to the reserved pad row and its contents must not leak past the override.
    model = _model(bs=1)
    runner = _runner(model)
...
def _runner_through_init(
    ...
    *,
    ...
    # Note (wenyao): mirrors ReqToTokenPool, whose indices come from [1, size] with
    # _alloc_size rows in total.
...
def test_startup_guard_rejects_tables_undersized_by_many_rows() -> None:
    # Note (wenyao): the guard is a bound check, not a "+1" check, so a table far
    # below the pool must be rejected too.
    with pytest.raises(RuntimeError, match="too small for the request pool"):
...
def test_startup_guard_logs_instead_of_silently_skipping(
    ...
    # Note (wenyao): an upstream rename of req_to_token_pool leaves every index site
    # working and only this check quiet, so the skip has to announce itself.
