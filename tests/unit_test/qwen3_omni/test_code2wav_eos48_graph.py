...
"""CPU checks for wider exact EOS coverage, not real CUDA decoder parity."""
...
def test_wider_single_optional_pool_covers_contiguous_range_with_same_budget(
    ...
    assert len(EXTRA48) == 43
    assert calls[0]["graph_memory_budget_bytes"] == 120
    assert optional.stats()["build"]["published_graph_count"] == 43
    assert optional.stats()["memory"]["graph_budget_bytes"] == 120
    assert backend.capture_calls == 43
    assert backend.warmup_iterations == [3] * 43
    ...
    # Changing inputs and keys, then replaying earlier inputs, catches stale
    # capture inputs with the real runner and existing fake CUDA backend.
...
def test_optional_replay_keeps_l5_snake_beta_guard(monkeypatch):
    scheduler, _, _, runners = _factory(monkeypatch)
    checks = []

    def guard():
        checks.append(torch.is_grad_enabled())
        if len(checks) > 1:
            raise RuntimeError("SnakeBeta static parameters changed")

    scheduler._snake_beta_guard = guard
    scheduler.decode_delta("a", _state(48), is_final=True)
    assert runners[1].calls[-1] == ((1, 2, 48), True, "cuda_graph")
    with pytest.raises(RuntimeError, match="SnakeBeta static parameters changed"):
        scheduler.decode_delta("b", _state(48), is_final=True)
    assert checks == [False, False]
    assert len(runners[1].calls) == 1
