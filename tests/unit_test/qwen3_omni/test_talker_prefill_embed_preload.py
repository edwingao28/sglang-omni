...
@pytest.mark.parametrize("mode", ["device", "cpu"])
def test_preload_rows_bitwise_match_lazy(model_dir, monkeypatch, mode):
...
@pytest.mark.parametrize("mode", ["device", "cpu"])
def test_preload_rows_bitwise_match_lazy_cuda(model_dir, monkeypatch, mode):
    def cuda_model():
        model = _fake_model()
...
@pytest.mark.parametrize("mode", ["device", "cpu"])
def test_preload_never_loads_rows_after_init(model_dir, monkeypatch, mode):
    monkeypatch.setenv("SGLANG_OMNI_TALKER_EMBED_PRELOAD", mode)
    builder = _build(model_dir)
...
def test_invalid_mode_is_off(model_dir, monkeypatch, caplog):
    monkeypatch.setenv("SGLANG_OMNI_TALKER_EMBED_PRELOAD", "gpu")
    with caplog.at_level("WARNING", logger=talker_prefill.__name__):
        builder = _build(model_dir)
    assert builder._embed_table is None
