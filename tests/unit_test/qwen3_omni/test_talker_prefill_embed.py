...
"""Single-token fast path of ``_load_prompt_token_embeddings``.

The streamed AR path calls it with exactly one token per text chunk; the fast
path must return the same row, bitwise, as the general unique/stack/gather path.
"""
