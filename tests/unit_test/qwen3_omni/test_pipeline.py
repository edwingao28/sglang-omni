...
def test_qwen_pipeline_config_and_state_contracts() -> None:
    ...
    # Thinker and talker_ar each declare the 3-way multimodal join directly —
    # no mm_aggregate relay hop on the speech critical path.
    ...
    # new_request via its own multimodal join so it can enter its deferred
    # state before the thinker finishes streaming. The thinker therefore only
    ...
    # Text-only pipeline keeps the mm_aggregate relay unchanged.
...
def test_qwen_preprocessing_routes_only_active_encoder_branches() -> None:
    ...
    for encoder_inputs, expected_next, expected_wait in cases:
        ...
        # Speech pipeline routes to the join consumers directly; the default
        # request (no output_modalities) generates audio, so talker_ar joins.
...
def test_qwen_merge_for_talker_matches_projected_thinker_merge() -> None:
    """merge_for_talker == project_mm_aggregate_to_talker_ar ∘ merge_for_thinker."""
    ...
    # Trimmed to the talker's needs: no encoder/mm leftovers travel cross-GPU.
