...
"""Admission staggering: pace concurrent submissions min-gap apart."""
...
def test_admission_gate_reserves_min_gap_slots() -> None:
    async def _run() -> None:
        ...
        # First slot is immediate; the remaining 3 wait 50ms apart.
        assert elapsed >= 0.15
        assert len(coordinator.control_plane.submitted) == 4
        # The next reserved slot sits one gap beyond the last admission.
...
def test_admission_gate_no_wait_for_spaced_arrivals() -> None:
    async def _run() -> None:
        ...
        # Arrival already past the reserved slot: no added delay.
